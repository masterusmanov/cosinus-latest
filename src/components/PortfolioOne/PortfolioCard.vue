<template>
    <div class="flex flex-wrap justify-center items-center gap-6 mt-[120px]">
        <div class="w-[98%] h-[480px] mx-auto rounded-[16px] bg-gradient-to-t from-[#999FB0] to-[#242B3D] p-0.5 md:w-[47%] md:h-[485px] lg:h-[650px] 2xl:h-[700px]">
            <div class="h-[475px] rounded-[16px] bg-[#171B3F] opacity-90 p-2 md:h-[480px] lg:h-[645px] 2xl:h-[695px]">
                <img src="../../assets/images/portfolio/dostavka.png" alt="" class="rounded-[16px] w-full">
                <div class="text-white font-sans font-[500] mt-[20px] ml-3 ">
                    <h2 class="text-[27px]">Yetkazib berish xizmati uchun tayyorlangan web sayt</h2>
                    <a href="#" class="flex items-center gap-3 mt-[20px] " style="color: var(--Medium-Aquamarine, #36C59E);">Web saytni ko’rish <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                        <path d="M15.6253 5V13.125C15.6253 13.2908 15.5595 13.4497 15.4423 13.5669C15.3251 13.6842 15.1661 13.75 15.0003 13.75C14.8346 13.75 14.6756 13.6842 14.5584 13.5669C14.4412 13.4497 14.3753 13.2908 14.3753 13.125V6.50859L5.44254 15.4422C5.32526 15.5595 5.1662 15.6253 5.00035 15.6253C4.8345 15.6253 4.67544 15.5595 4.55816 15.4422C4.44088 15.3249 4.375 15.1659 4.375 15C4.375 14.8341 4.44088 14.6751 4.55816 14.5578L13.4918 5.625H6.87535C6.70959 5.625 6.55062 5.55915 6.43341 5.44194C6.3162 5.32473 6.25035 5.16576 6.25035 5C6.25035 4.83424 6.3162 4.67527 6.43341 4.55806C6.55062 4.44085 6.70959 4.375 6.87535 4.375H15.0003C15.1661 4.375 15.3251 4.44085 15.4423 4.55806C15.5595 4.67527 15.6253 4.83424 15.6253 5Z" fill="#36C59E"/>
                        </svg></a>
                </div>
            </div>
        </div>
        <div class="w-[98%] h-[480px] mx-auto rounded-[16px] bg-gradient-to-t from-[#999FB0] to-[#242B3D] p-0.5 md:w-[47%] md:h-[485px] lg:h-[650px] 2xl:h-[700px]">
            <div class="h-[475px] rounded-[16px] bg-[#171B3F] opacity-90 p-2 md:h-[480px] lg:h-[645px] 2xl:h-[695px]">
                <img src="../../assets/images/portfolio/shifoweb.png" alt="" class="rounded-[16px] w-full">
                <div class="text-white font-sans font-[500] mt-[20px] ml-3 ">
                    <h2 class="text-[27px]">Shifoxona uchun tayyorlangan Shifo24 web-sayti</h2>
                    <a href="#" class="flex items-center gap-3 mt-[20px] " style="color: var(--Medium-Aquamarine, #36C59E);">Web saytni ko’rish <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                        <path d="M15.6253 5V13.125C15.6253 13.2908 15.5595 13.4497 15.4423 13.5669C15.3251 13.6842 15.1661 13.75 15.0003 13.75C14.8346 13.75 14.6756 13.6842 14.5584 13.5669C14.4412 13.4497 14.3753 13.2908 14.3753 13.125V6.50859L5.44254 15.4422C5.32526 15.5595 5.1662 15.6253 5.00035 15.6253C4.8345 15.6253 4.67544 15.5595 4.55816 15.4422C4.44088 15.3249 4.375 15.1659 4.375 15C4.375 14.8341 4.44088 14.6751 4.55816 14.5578L13.4918 5.625H6.87535C6.70959 5.625 6.55062 5.55915 6.43341 5.44194C6.3162 5.32473 6.25035 5.16576 6.25035 5C6.25035 4.83424 6.3162 4.67527 6.43341 4.55806C6.55062 4.44085 6.70959 4.375 6.87535 4.375H15.0003C15.1661 4.375 15.3251 4.44085 15.4423 4.55806C15.5595 4.67527 15.6253 4.83424 15.6253 5Z" fill="#36C59E"/>
                        </svg></a>
                </div>
            </div>
        </div>
        <div class="w-[98%] h-[480px] mx-auto rounded-[16px] bg-gradient-to-t from-[#999FB0] to-[#242B3D] p-0.5 md:w-[47%] md:h-[485px] lg:h-[650px] 2xl:h-[700px]">
            <div class="h-[475px] rounded-[16px] bg-[#171B3F] opacity-90 p-2 md:h-[480px] lg:h-[645px] 2xl:h-[695px]">
                <img src="../../assets/images/portfolio/shifomobile.png" alt="" class="rounded-[16px] w-full">
                <div class="text-white font-sans font-[500] mt-[20px] ml-3 ">
                    <h2 class="text-[27px]">Shifoxona uchun tayyorlangan Shifo24 mobile dasturi</h2>
                    <a href="#" class="flex items-center gap-3 mt-[20px] " style="color: var(--Medium-Aquamarine, #36C59E);">Web saytni ko’rish <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                        <path d="M15.6253 5V13.125C15.6253 13.2908 15.5595 13.4497 15.4423 13.5669C15.3251 13.6842 15.1661 13.75 15.0003 13.75C14.8346 13.75 14.6756 13.6842 14.5584 13.5669C14.4412 13.4497 14.3753 13.2908 14.3753 13.125V6.50859L5.44254 15.4422C5.32526 15.5595 5.1662 15.6253 5.00035 15.6253C4.8345 15.6253 4.67544 15.5595 4.55816 15.4422C4.44088 15.3249 4.375 15.1659 4.375 15C4.375 14.8341 4.44088 14.6751 4.55816 14.5578L13.4918 5.625H6.87535C6.70959 5.625 6.55062 5.55915 6.43341 5.44194C6.3162 5.32473 6.25035 5.16576 6.25035 5C6.25035 4.83424 6.3162 4.67527 6.43341 4.55806C6.55062 4.44085 6.70959 4.375 6.87535 4.375H15.0003C15.1661 4.375 15.3251 4.44085 15.4423 4.55806C15.5595 4.67527 15.6253 4.83424 15.6253 5Z" fill="#36C59E"/>
                        </svg></a>
                </div>
            </div>
        </div>
        <div class="w-[98%] h-[480px] mx-auto rounded-[16px] bg-gradient-to-t from-[#999FB0] to-[#242B3D] p-0.5 md:w-[47%] md:h-[485px] lg:h-[650px] 2xl:h-[700px]">
            <div class="h-[475px] rounded-[16px] bg-[#171B3F] opacity-90 p-2 md:h-[480px] lg:h-[645px] 2xl:h-[695px]">
                <img src="../../assets/images/portfolio/barbershop.png" alt="" class="rounded-[16px] w-full">
                <div class="text-white font-sans font-[500] mt-[20px] ml-3 ">
                    <h2 class="text-[27px]">Barbershop uchun tayyorlangan web-sayt</h2>
                    <a href="#" class="flex items-center gap-3 mt-[20px] " style="color: var(--Medium-Aquamarine, #36C59E);">Web saytni ko’rish <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                        <path d="M15.6253 5V13.125C15.6253 13.2908 15.5595 13.4497 15.4423 13.5669C15.3251 13.6842 15.1661 13.75 15.0003 13.75C14.8346 13.75 14.6756 13.6842 14.5584 13.5669C14.4412 13.4497 14.3753 13.2908 14.3753 13.125V6.50859L5.44254 15.4422C5.32526 15.5595 5.1662 15.6253 5.00035 15.6253C4.8345 15.6253 4.67544 15.5595 4.55816 15.4422C4.44088 15.3249 4.375 15.1659 4.375 15C4.375 14.8341 4.44088 14.6751 4.55816 14.5578L13.4918 5.625H6.87535C6.70959 5.625 6.55062 5.55915 6.43341 5.44194C6.3162 5.32473 6.25035 5.16576 6.25035 5C6.25035 4.83424 6.3162 4.67527 6.43341 4.55806C6.55062 4.44085 6.70959 4.375 6.87535 4.375H15.0003C15.1661 4.375 15.3251 4.44085 15.4423 4.55806C15.5595 4.67527 15.6253 4.83424 15.6253 5Z" fill="#36C59E"/>
                        </svg></a>
                </div>
            </div>
        </div>
    </div>
</template>

<script setup>

</script>

<style lang="scss" scoped>

</style>